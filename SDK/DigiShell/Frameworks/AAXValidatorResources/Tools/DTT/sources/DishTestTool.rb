...
require 'win32ole' unless /apple|linux/.match(eval("#{config}::CONFIG['host']"))

module DishTestTool

  # :stopdoc:
  # If we got here, our folder must be in the lib path.
  SOURCE_DIR = Pathname.new(__FILE__).parent.realpath
  ...
  # Paths for DSH log collection
  RELEASE_DSH_LOGS_PATH = ENV["HOME"]+"/Library/Logs/Avid/" if RUNTIME_INFO.mac?
  DEBUG_DSH_LOGS_PATH = SOURCE_DIR + "../../CommandLineTools/Logs/" if RUNTIME_INFO.mac?
  #TODO: add corresponding DSH logs paths for Win
