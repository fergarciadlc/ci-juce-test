...
module DishTestTool
  module AAXHDishUtils
    ...
    STEM_FORMAT = {
        ...
        :'7_0_2'    => '7p0p2',
        :'7_1_2'    => '7p1p2',
        :'ambisonics_(1st_order)' => 'ambi1st',
        :'ambisonics_(2nd_order)' => 'ambi2nd',
        :'ambisonics_(3rd_order)' => 'ambi3rd'
    }
    ...
    CONVENTIONS_ID_LITERAL = {
        PROP_PROD_ID                => [nil, :prod_id],
        PROP_MANU_ID                => [nil, :manu_id]
    }

    def grab_effect_literal_ids(plugin_id, effect_id)
      info = {}
      info[:name] = nil
      ...
      if description['property_map']
        description['property_map']['property'].each do |prop|
          ...
          extract_id_literal_property(prop, info)
        end
      end
      
      #fill in ids if they were not reported at the top level 
      if info[:prod_id].nil? or info[:manu_id].nil?
        #changing root
          ...
          proc_set_list.each do |proc_set|
            proc_set['proc_descriptor'].each do |descriptor|
              ...
              descriptor['properties']['property_map']['property'].each do |property|
                ...
                extract_id_literal_property(property, sub_collector)
              end #property
              
              #Updating :prod_id and :manu_id
              info[:prod_id] = sub_collector[:prod_id] if info[:prod_id].nil?
              info[:manu_id] = sub_collector[:manu_id] if info[:manu_id].nil?
            end
          end
        end
      end
    ...
    def grab_effect_info(plugin_id, effect_id)
      ...
      # Remove unsupported (reserved) stem-formats if any
      dsp_types.delete_if {|e| e[:input_sf].nil? || e[:out_sf].nil? }
      native_types.delete_if {|e| e[:input_sf].nil? || e[:out_sf].nil? }
...
=end
    def to_stem_format(int)
      ...
      case int
        ...
        when 20 then '7_0_2'
        when 13 then '7_1_2'
        when 14 then 'ambisonics(1st_order)'
        when 18 then 'ambisonics(2nd_order)'
        when 19 then 'ambisonics(3rd_order)'           
        when -1 then 'Any'
      end
    end
    ...
    def extract_id_literal_property(property, extracted = {})
      desired_keys = CONVENTIONS_ID_LITERAL.keys & property.values
      desired_keys.each do |key|
        prop_key, prop_method = CONVENTIONS_ID_LITERAL[key][1], CONVENTIONS_ID_LITERAL[key][0]
        extracted[prop_key] = prop_method.nil? ? property['val_literal'] : send(prop_method, property['val_literal'])
      end #desired_keys
    end #extract_id_literal_property
