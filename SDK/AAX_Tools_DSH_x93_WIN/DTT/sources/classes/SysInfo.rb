    ...
    def load_body
      ...
      if @rt_info.win?
        ...
        Pathname.new("/Volumes").children.each do |volume|
          diskutil_info = {}
          `diskutil info \"#{volume.to_s}\"`.split("\n").each do |line|
        ...
        disks_overview.map do |disk|
          ...
          disk['Volume Free Space'] = disk['Volume Free Space'][/^(\d*\.?\d*\s\w+)/]
        ...
        boot_node = `bless --info --getBoot`.chomp
        boot_id = disks_overview.find{ |d| d['Device Node'].eql?(boot_node)}
        @boot_id = boot_id ? boot_id['Mount Point'] : 'Cannot identify boot partition'
      end
      return self
    end
  end # Sys_Info
